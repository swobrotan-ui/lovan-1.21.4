...
public class ModeComponent extends AbstractComponent {
    ...
    public void render(MatrixStack matrixStack, float x, float y) {
        ...
        DrawHelper.drawVerticalCenteredText(matrix, mediumFont, setting.getSelected(), x + 10 + 8, rectY, 28, new Color(102, 101, 110));
    }
    ...
    public void renderDropdown(MatrixStack matrixStack) {
        ...
        Color bgColor = new Color(21, 22, 29, (int)(alpha * 255));
        
        DrawHelper.drawRect(matrixStack, componentX + 10, dropdownY, 235, dropdownHeight, 6, bgColor);
        
        for (int i = 0; i < modes.size(); i++) {
            String mode = modes.get(i);
            Color baseColor = mode.equals(setting.getSelected()) ? 
        ...
        if (button == 0 && mouseX >= componentX + 10 && mouseX <= componentX + 10 + 235) {
            ...
            for (int i = 0; i < modes.size(); i++) {
                ...
                if (mouseY >= itemY && mouseY <= itemY + 28) {
                    setting.setSelected(modes.get(i));
                    expanded = false;
                    return true;
                }
            }
