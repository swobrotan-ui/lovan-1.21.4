...
public class RenderUtil implements IMinecraft {
    ...
    private static final Supplier<SimpleFramebuffer> MOTION_PREV_FBO_SUPPLIER = Suppliers.memoize(() -> new SimpleFramebuffer(mc.getWindow().getFramebufferWidth(), mc.getWindow().getFramebufferHeight(), false));
    private static final Supplier<SimpleFramebuffer> MOTION_CUR_FBO_SUPPLIER = Suppliers.memoize(() -> new SimpleFramebuffer(mc.getWindow().getFramebufferWidth(), mc.getWindow().getFramebufferHeight(), false));

    private static long frameId = 0L;
    private static long blurCachedFrameId = -1L;
    ...
    public static void drawBlur(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float blurRadius, int color) {
        ...
        enableRender();
        if (blurCachedFrameId != frameId) {
            blurCachedFrameId = frameId;
        ...
        GL11.glTexParameterfv(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_BORDER_COLOR, new float[]{0f, 0f, 0f, 0f});
    ...
    private static Matrix4f matricesIdentity() {
    ...
    public static void drawTexture(MatrixStack matrices, Object texture, float x, float y, float width, float height, float rounding, int color) {
        ...
        if (texture instanceof String path) {
            textureId = Identifier.of("sodiumextra", path);
