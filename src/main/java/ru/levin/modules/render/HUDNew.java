...
@FunctionAnnotation(name = "HUDNew", desc = "Интерфейс клиента", type = Type.Render)
public class HUDNew extends Function {
    ...
    public final MultiSetting setting = new MultiSetting(
            "Элементы",
            Arrays.asList("WaterMarkNew", "HUDNew", "TargetHUDNew", "KeyBinds", "StaffList", "PotionHUDNew", "ItemCoolDownHUD", "Coordinates / TPS","ArmorHUDNew", "Notifications", "Медиаплеер (мощные пк)", "SwapHelpers", "ActiveMods", "Sensitivity"),
            new String[]{"WaterMarkNew", "HUDNew", "TargetHUDNew", "KeyBinds", "StaffList", "PotionHUDNew", "ItemCoolDownHUD", "Coordinates / TPS","ArmorHUDNew", "Notifications", "Медиаплеер (мощные пк)", "SwapHelpers", "ActiveMods", "Sensitivity"});
    ...
    private final ModeSetting hudStyle = new ModeSetting(
            () -> setting.get("WaterMarkNew") || setting.get("HUDNew"),
            "Стиль HUD",
            "Водяной",
            "Водяной",
            "Инфобар"
    ...
    public final BooleanSetting visibleCrosshair = new BooleanSetting("Показывать TargetHUD при навидении", false, "показывает таргетхуд при навидении на игрока", () -> setting.get("TargetHUDNew"));
    ...
    public final Dragging watermarkDrag = ExosWare.getInstance().createDrag(this, "WaterMarkNew", 10, 10);
    public final Dragging targethudDrag = ExosWare.getInstance().createDrag(this, "TargetHUDNew", 10, 45);
    public final Dragging keybindsDrag = ExosWare.getInstance().createDrag(this, "KeyBindsHUDNew", 10, 95);
    public final Dragging stafflistDrag = ExosWare.getInstance().createDrag(this, "StaffListHUDNew", 10, 128);
    public final Dragging helperHudDrag = ExosWare.getInstance().createDrag(this, "HelperHUDNew", 10, 158);
    public final Dragging itemcooldownDrag = ExosWare.getInstance().createDrag(this, "CoolDownHUDNew", 10, 165);
    public final Dragging potionhudDrag = ExosWare.getInstance().createDrag(this, "PotionHUDNew", 10, 198);
    public final Dragging coordinateshudDrag = ExosWare.getInstance().createDrag(this, "CoordinatesHUDNew", 10, 198);
    public final Dragging armorDrag = ExosWare.getInstance().createDrag(this, "ArmorHUDNew", 478, 468);
    public final Dragging notificationsDrag = ExosWare.getInstance().createDrag(this, "NotificationsHUDNew", 10, 260);
    public final Dragging mediaPlayerDrag = ExosWare.getInstance().createDrag(this, "MediaPlayerHUDNew", 10, 290);
    public final Dragging swapHelpersDrag = ExosWare.getInstance().createDrag(this, "SwapHelpersHUDNew", 10, 320);
    public final Dragging mouseSensDrag = ExosWare.getInstance().createDrag(this, "MouseSensHUDNew", 10, 380);
...
private boolean mediaPlayerDraggingProgress = false;
    ...
    private float[] audioBars = new float[5];
    private float audioBarTarget = 0f;
    private long lastAudioUpdateMs = 0L;
    ...
    private float getMusicVolume() {
        ...
        if (media == null || media.getTitle() == null || media.getTitle().isEmpty()) return 0f;
        isMusicPlaying = true;
        // Simulate volume-based bar heights using track position as a seed
        long pos = media.getPosition();
        float base = (float) (Math.sin(pos * 0.001) * 0.5 + 0.5);
        float variation = (float) (Math.sin(pos * 0.003) * 0.3 + Math.cos(pos * 0.007) * 0.2);
        return MathHelper.clamp(base + variation, 0.1f, 1.0f);
    }

    private void updateAudioBars() {
        long now = System.currentTimeMillis();
        if (now - lastAudioUpdateMs < 50L) return;
        lastAudioUpdateMs = now;

        float targetVolume = getMusicVolume();
        audioBarTarget = targetVolume;

        for (int i = 0; i < audioBars.length; i++) {
            float phase = (float) (now * 0.008 + i * 0.3);
            float target = targetVolume * (0.5f + 0.5f * (float) Math.sin(phase));
            audioBars[i] += (target - audioBars[i]) * 0.3f;
            if (Math.abs(audioBars[i]) < 0.02f && targetVolume < 0.05f) {
                audioBars[i] = 0f;
            }
        }
    }
 ...
 if (event instanceof EventMouse mouse) {
  ...
  if (event instanceof EventUpdate) {
        ...
        if (event instanceof EventRender2D eventRender2D) {
            ...
            boolean sWaterMark = setting.get("WaterMarkNew");
            boolean sTargetHUD = setting.get("TargetHUDNew");
            ...
            boolean sPotion = setting.get("PotionHUDNew");
            boolean sCoordinates = setting.get("Coordinates / TPS");
            boolean sArmorHUD = setting.get("ArmorHUDNew");
            ...
            if (sWaterMark && hudStyle.is("Водяной")) hudNewWaterMark(eventRender2D);
            ...
            if (sMediaPlayer) hudNewMediaPlayer(eventRender2D);
    ...
    private void hudNewOnEvent(Event event) {
        ...
        if (event instanceof EventRender2D eventRender2D) {
            boolean sWaterMark = setting.get("WaterMarkNew");
            boolean sMediaPlayer = setting.get("Медиаплеер (мощные пк)");
            if (sWaterMark) hudNewWaterMark(eventRender2D);
            if (sMediaPlayer) hudNewMediaPlayer(eventRender2D);
        }
    }
    ...
    public void hudNewMediaPlayer(EventRender2D eventRender2D) {
    ...
    public void hudNewWaterMark(EventRender2D render2D) {
...
long now = System.currentTimeMillis();
        ...
        if (watermarkPosition.is("По центру")) {
            ...
            // Audio visualization bars when music is playing
            if (isMusicPlaying && audioBarTarget > 0.05f) {
                float barAreaWidth = pillW - 20f;
                if (barAreaWidth > 30f) {
                    float barX = x + (pillW - barAreaWidth) / 2f;
                    float barY = y + pillH / 2f - 3f;
                    float barWidth = Math.max(1f, (barAreaWidth - 12f) / 5f);
                    float barGap = 2f;
                    float maxBarH = 6f;
                    for (int i = 0; i < 5; i++) {
                        float bh = Math.max(1f, audioBars[i] * maxBarH);
                        int barColor = isMono
                                ? new Color(180, 180, 180, 200).getRGB()
                                : hudAccent(90, 200);
                        drawRoundedRect(matrices, barX + i * (barWidth + barGap), barY + (maxBarH - bh), barWidth, bh, new Vector4f(1f, 1f, 1f, 1f), barColor);
                    }
                }
            }
