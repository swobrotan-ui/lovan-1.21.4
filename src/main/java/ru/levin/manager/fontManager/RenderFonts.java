...
public class RenderFonts implements Closeable, IMinecraft {
    ...
    private class GlyphMap {
        ...
        private void generate() {
            ...
            // getStringBounds(" ") у некоторых шрифтов/настроек может давать ширину 0,
            // из-за чего пробелы в GUI визуально "пропадают" (текст слипается).
            // Для таких символов берём метрики из FontMetrics.
            for (int i = 0; i < metrics.length; i++) {
                CharMetrics cm = metrics[i];
                if (cm.width <= 0) {
                    int w = fm.charWidth(cm.character);
                    metrics[i] = new CharMetrics(cm.character, Math.max(w, 1), cm.height);
                }
                if (metrics[i].height <= 0) {
                    metrics[i] = new CharMetrics(metrics[i].character, metrics[i].width, Math.max(fm.getHeight(), 1));
                }
            }
