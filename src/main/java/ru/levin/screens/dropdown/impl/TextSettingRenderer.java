...
public class TextSettingRenderer implements SettingRenderer<TextSetting> {
    ...
    public void render(DrawContext ctx, TextSetting setting, int x, int y, int width, int height) {
        int fontSizeName = 12;
        int fontSizeField = 12;

        int nameHeight = (int) FontUtils.sf_medium[fontSizeName].getHeight();
        FontUtils.sf_medium[fontSizeName].drawLeftAligned(ctx.getMatrices(), setting.getName(), x + PADDING, y, Color.WHITE.getRGB());

        String value = setting.getValue();
        int textWidth = (int) FontUtils.sf_medium[fontSizeField].getWidth(value) + HORIZONTAL_PADDING * 2;

        int fieldWidth = Math.max(MIN_FIELD_WIDTH, Math.min(textWidth, MAX_FIELD_WIDTH));
        int fieldHeight = (int) (FontUtils.sf_medium[fontSizeField].getHeight() + VERTICAL_PADDING * 2);
        int fieldX = x + PADDING;
        int fieldY = y + nameHeight + 2;

        int themeBase = Manager.STYLE_MANAGER.getFirstColor();
        int focusColor = ColorUtil.reAlphaInt(themeBase, 128);
        int bgColor = setting.isFocused() ? focusColor : new Color(40, 40, 40, 225).getRGB();
        ...
        int cursorX = (int) FontUtils.sf_medium[fontSizeField].getWidth(value.substring(0, setting.getCursorPosition()));
        ...
        FontUtils.sf_medium[fontSizeField].drawLeftAligned(ctx.getMatrices(), value, fieldX + HORIZONTAL_PADDING + scrollOffset, fieldY + VERTICAL_PADDING - 0.6f, Color.WHITE.getRGB());
        Scissor.pop();
        ...
        if (setting.isFocused()) {
            ...
            if (setting.cursorVisible) {
                float cursorPos = FontUtils.sf_medium[fontSizeField].getWidth(value.substring(0, setting.getCursorPosition()));
                RenderUtil.drawRoundedRect(ctx.getMatrices(), fieldX + HORIZONTAL_PADDING + cursorPos + scrollOffset, fieldY + VERTICAL_PADDING, 1, FontUtils.sf_medium[fontSizeField].getHeight(), 0, Color.WHITE.getRGB());
            }
        }
    }
    ...
    public boolean mouseClicked(TextSetting setting, double mouseX, double mouseY, int button, int x, int y, int width, int height) {
        ...
        int fontSizeField = 16;
        int nameHeight = (int) FontUtils.sf_medium[fontSizeField].getHeight();
        int fieldY = y + nameHeight + 2;
        String value = setting.getValue();
        int fieldWidth = Math.max(MIN_FIELD_WIDTH, Math.min((int) FontUtils.sf_medium[fontSizeField].getWidth(value) + HORIZONTAL_PADDING * 2, MAX_FIELD_WIDTH));
        int fieldHeight = (int) (FontUtils.sf_medium[fontSizeField].getHeight() + VERTICAL_PADDING * 2);
        int fieldX = x + PADDING;
    ...
    public int getHeight() {
        int fontSizeName = 16;
        int nameHeight = (int) FontUtils.sf_medium[fontSizeName].getHeight();
        int fieldHeight = (int) (FontUtils.sf_medium[16].getHeight() + VERTICAL_PADDING * 3f);
