...
public class BindBooleanSettingRenderer implements SettingRenderer<BindBooleanSetting>, IMinecraft {
    ...
    public void render(DrawContext ctx, BindBooleanSetting setting, int x, int y, int width, int height) {
        ...
        if (setting.expanded) {
            ...
            float textWidth = FontUtils.sf_medium[16].getWidth(displayText);
            int bindBoxWidth = (int) (10 + textWidth);
            int bindBoxHeight = 16;
            ...
            RenderUtil.drawRoundedRect(ctx.getMatrices(), bindBoxX, bindBoxY, bindBoxWidth, bindBoxHeight, 3, new Color(40, 40, 40, 225).getRGB());
            ...
            FontUtils.sf_medium[16].drawLeftAligned(ctx.getMatrices(), displayText, bindBoxX + 5 - offset, bindBoxY + (bindBoxHeight - FontUtils.sf_medium[16].getHeight()) / 2f, Color.WHITE.getRGB());

            int closeSize = 12;
            int paddingRightClose = 8;
            int closeX = x + width - closeSize - paddingRightClose + 10;
            int closeY = bindBoxY + (bindBoxHeight - closeSize) / 2 - 1;
            RenderUtil.drawRoundedRect(ctx.getMatrices(), closeX, closeY, closeSize, closeSize, 3, new Color(80, 30, 30, 235).getRGB());
            FontUtils.sf_medium[16].centeredDraw(ctx.getMatrices(), "x", closeX + closeSize / 2f, closeY + 1.5f, Color.WHITE.getRGB());
        ...
        int switchY = y + (HEIGHT - SWITCH_HEIGHT) / 2 - 1;
        Color offColor = new Color(50, 50, 50, 225);
        int themeBase = Manager.STYLE_MANAGER.getFirstColor();
        int onColor = ColorUtil.reAlphaInt(themeBase, 128);
        ...
        int textX = x;
        int textY = (int) (y + (HEIGHT - FontUtils.sf_medium[16].getHeight()) / 2f) - 1;
        String text = setting.getName();
        float textWidth = FontUtils.sf_medium[16].getWidth(text);
        ...
        Scissor.push();
        Scissor.setFromComponentCoordinates(textX, textY - 1, maxTextWidth, FontUtils.sf_medium[16].getHeight() + 2);
        FontUtils.sf_medium[16].drawLeftAligned(ctx.getMatrices(), text, textX - offset, textY, Color.WHITE.getRGB());
        Scissor.pop();
    ...
    public boolean mouseClicked(BindBooleanSetting setting, double mouseX, double mouseY, int button, int x, int y, int width, int height) {
        ...
        int gearY = y + (HEIGHT / 2) - gearSize / 2 - 1;
        ...
        if (setting.expanded) {
            ...
            float textWidth = FontUtils.sf_medium[16].getWidth(setting.isListeningForBind() ? "Binding" : (setting.getBindKey() != -1 ? ClientManager.getKey(setting.getBindKey()) : "NONE"));
            int bindBoxWidth = (int) (10 + textWidth);
            int bindBoxHeight = 16;
            int bindBoxTargetX = x - 2;
            int bindBoxX = (int) (bindBoxTargetX - (bindBoxWidth + 10) * (1 - bindBoxProgress));
