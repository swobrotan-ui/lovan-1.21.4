...
public class BindSettingRenderer implements SettingRenderer<BindSetting>, IMinecraft {
    ...
    public void render(DrawContext ctx, BindSetting setting, int x, int y, int width, int height) {
        ...
        int fontSizeName = 16;
        int fontSizeButton = 16;

        int textWidthButton = (int) FontUtils.sf_medium[fontSizeButton].getWidth(displayText);
        ...
        float nameTextWidth = FontUtils.sf_medium[fontSizeName].getWidth(name);
        ...
        Scissor.push();
        Scissor.setFromComponentCoordinates(nameAreaX, y + (HEIGHT - FontUtils.sf_medium[fontSizeName].getHeight()) / 2 - 1, nameAreaWidth, FontUtils.sf_medium[fontSizeName].getHeight() + 2);
        FontUtils.sf_medium[fontSizeName].drawLeftAligned(ctx.getMatrices(), name, nameAreaX - scrollOffset, y + (HEIGHT - FontUtils.sf_medium[fontSizeName].getHeight()) / 2, Color.WHITE.getRGB());
        Scissor.pop();

        RenderUtil.drawRoundedRect(ctx.getMatrices(), buttonX, buttonY, buttonWidth, buttonHeight, BUTTON_RADIUS, new Color(40, 40, 40, 225).getRGB());

        FontUtils.sf_medium[fontSizeButton].centeredDraw(ctx.getMatrices(), displayText, buttonX + buttonWidth / 2f, buttonY + (buttonHeight - FontUtils.sf_medium[fontSizeButton].getHeight()) / 2f, Color.WHITE.getRGB());
    }
