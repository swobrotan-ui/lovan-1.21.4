    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (RenderUtil.isInRegion(mouseX, mouseY, randomX, buttonsY, buttonWidth, inputHeight) && button == 0) {
            String[] words = new String[]{
                    "princess", "shadow", "phantom", "dragon", "hawk",
                    "wolf", "storm", "blaze", "frost", "thunder",
                    "night", "soul", "ace", "king", "ghost",
                    "viper", "ninja", "hero", "mage", "spark"
            };
            String randomWord = words[ThreadLocalRandom.current().nextInt(words.length)];
            String randomName = "lovan_" + randomWord;
